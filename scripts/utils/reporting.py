...
def exec_extra_action(detection: Detection):
    global eec_time
    conf=get_settings()
    # read EEC settings from birdnet.conf
    ...
    # see if it's the right species and 5 mins have elapsed since last action
    if com_name.upper().find(conf["EEC_DETECT"].upper()) >= 0 and (time.time()-eec_time >=300):
